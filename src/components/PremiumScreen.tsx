...
export const PremiumScreen: React.FC<PremiumScreenProps> = ({ onBack }) => {
  ...
  const benefits = [
    { icon: <MessageSquare size={20} />, text: "Análises ilimitadas", desc: "Sem o limite de apenas 2 análises" },
  ...
  return (
    <div className="flex flex-col items-center px-5 py-8 sm:py-12 max-w-lg mx-auto overflow-y-auto h-full">
      <motion.div 
        initial={{ opacity: 0, x: -20 }}
        animate={{ opacity: 1, x: 0 }}
        className="w-full flex justify-start mb-2 sm:mb-4"
      >
        <button 
          onClick={onBack}
          className="p-3 rounded-full bg-white/5 hover:bg-white/10 transition-colors text-gray-400 group"
        >
      ...
      >
        <div className="inline-block p-1 rounded-full bg-gradient-to-r from-brand-purple to-brand-pink mb-3 sm:mb-4">
          <div className="bg-dark-bg rounded-full px-3 py-1 flex items-center gap-1.5 sm:gap-2">
            <Star size={14} className="text-brand-pink fill-brand-pink" />
            <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider">Premium Access</span>
          </div>
        </div>
